class AFQuestao4 {
    fun main() {
        ...
        println("Lucro com a venda de todos os itens: R\$ $lucroTotal")
        ...
        mostrarEstoque(estoqueInicial)
        println("Prejuízo com a retirada dos itens vencidos: R\$ $prejuizo")
    ...
    private fun calcularLucro(estoque: Estoque): Double {
        ...
        val lucroTotal = itens.sumByDouble { it.getProduto().getPreco() }
        return lucroTotal
    }
    ...
    private fun retirarItensVencidos(estoque: Estoque): Double {
        ...
        for (item in itens) {
            if (!item.getValidade().depois(dataAtual)) {
